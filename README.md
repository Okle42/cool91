...
**5. 風扇忽高忽低**
單次取樣的 CPU 最高溫抖動很大（89 → 82 → 84）。第一版用對稱 EMA（α = 0.5）+ 100 rpm deadband，還是會 4900 → 4460 → 4700 → 4900 來回跳。現在升溫用 α = 0.7 快反應、降溫用 α = 0.2 慢慢放，再加斜率限制：每輪最多降 300 rpm、升 800 rpm（預熱與剛接管時不限）。從全速降到最低至少 65 秒，風扇不會被反覆抽動；升速也限一下是因為階段性負載（算一段、鬆一下、再算一段）會讓風扇「忽然大聲 → 慢慢小聲 → 忽然大聲」，限了之後聲音變化平順，散熱幾乎沒差（3000 → 4900 只要 12 秒）。
