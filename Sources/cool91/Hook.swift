...
func runHook(config: Config) {
    ...
    // 備註只寫命中的關鍵字，不寫指令原文：log 是 644 全機可讀，指令裡可能有 token、私人路徑
    if !command.isEmpty, let k = Policy.boostKeyword(command, keywords: config.boostCommands) {
        Event(kind: .boost, rpm: config.boostRPM, seconds: config.boostSeconds, note: k).post()
    }
